...
# [当前流程未使用] 仅当显式调用 Pipeline.create_bm25_db 时使用，process_parsed_reports 未调用
...
class VectorDBIngestor:
    ...
    def process_reports(self, all_reports_dir: Path, output_dir: Path):
        ...
        for report_path in tqdm(all_report_paths, desc="处理Faiss向量库"):
            # 加载报告
            with open(report_path, 'r', encoding='utf-8') as f:
                report_data = json.load(f)
...
if __name__ == "__main__":
    vector_db_ingestor = VectorDBIngestor()
    vector_db_ingestor.process_reports(Path("data/stock_data/databases/chunked_reports"), Path("data/stock_data/databases/vector_dbs"))
    print("Faiss向量库处理完成")