"""
集中维护项目中所有 Prompt（system/user）与其结构化输出 Schema（Pydantic）。

说明：
- 本文件会被 `src/api_requests.py`、`src/reranking.py` 等模块引用。
- 本次仅做排版/分区增强，避免影响对外接口（类名、字段名、引用路径保持不变）。
"""
...
# 无检索上下文时使用的默认系统提示（仅无公司/无文档时使用，JSON 格式与正常流程一致）聊天模式
DEFAULT_SYSTEM_PROMPT = (
    "你是一个智能问答助手，可以回答用户提出的问题，利用和用户的历史对话理解用户的追问。语气要尽量地亲切、自然、友好。"
)

class SourceRef(BaseModel):
    """单个引用来源：文件名 + 行号范围"""
    file_name: str = Field(description="文件名，与上下文中检索到的来源一致，如 '【国信证券】xxx.md'")
    line_start: int = Field(description="起始行号（从1开始）")
    line_end: int = Field(description="结束行号（从1开始）")
...
# =============================================================================
# Comparative question rewriting
# =============================================================================
...
# =============================================================================
# RAG answer prompts (shared base)
# =============================================================================

class AnswerWithRAGContextSharedPrompt:
    instruction = """
你是一个RAG（检索增强生成）问答系统。
你的任务是基于公司年报中RAG检索到的相关片段内容（每段会标明「文件名」和「行号范围」），回答给定问题。
如果[上下文]中没有相关信息，请回答‘我不知道’。

在给出最终答案前，请详细分步思考，尤其关注问题措辞。
- 注意：答案可能与问题表述不同。
- 问题可能是模板生成的，有时对该公司不适用。
- 引用时请使用上下文中出现的「文件名」和「行号范围」，填入 relevant_sources。
"""
...
# =============================================================================
# RAG answer prompts (by output schema)
# =============================================================================
...
class AnswerWithRAGContextNamePrompt:
    ...
    class AnswerSchema(BaseModel):
        ...
        relevant_sources: List[SourceRef] = Field(description="""
仅包含直接用于回答问题的信息来源，每项为「文件名 + 行号范围」，必须与上下文中出现的完全一致。只包括：
- 直接包含答案或明确陈述的片段（其 file_name 与 line_start/line_end）
- 强有力支持答案的关键信息片段
不要包含仅与答案弱相关或间接相关的来源。列表中至少应有一项。
""")
...
```
{
  ...
  "relevant_sources": [{"file_name": "南方航空2024年年度报告.md", "line_start": 58, "line_end": 62}],
  "final_answer": "张三"
}
```
""" 
...
""")
        ...
        relevant_sources: List[SourceRef] = Field(description="""
仅包含直接用于回答问题的信息来源（文件名+行号范围），与上下文中出现的完全一致。只包括：
- 直接包含答案或明确陈述的片段
- 强有力支持答案的关键信息片段。列表中至少应有一项。
""")
...
```
{
  ...
  "relevant_sources": [{"file_name": "万科企业2022年年度报告.md", "line_start": 76, "line_end": 80}],
...
```
{
  ...
  "relevant_sources": [{"file_name": "某医药公司年报.md", "line_start": 33, "line_end": 39}, {"file_name": "某医药公司年报.md", "line_start": 35, "line_end": 38}],
  "final_answer": "N/A"
}
```
"""
...
""")
        reasoning_summary: str = Field(description="简要总结分步推理过程，约50字。")
        relevant_sources: List[SourceRef] = Field(description="""
仅包含直接用于回答问题的信息来源（文件名+行号范围），与上下文中出现的完全一致。列表中至少应有一项。
""")
...
```
{
  ...
  "relevant_sources": [{"file_name": "某公司年报.md", "line_start": 12, "line_end": 15}, {"file_name": "某公司年报.md", "line_start": 18, "line_end": 20}],
  "final_answer": false
}
```
"""
...
```
{
    ...
    "relevant_sources": [{"file_name": "某公司年报.md", "line_start": 87, "line_end": 92}],
    "final_answer": ["张三", "李四"]
}
```
"""
...
"""
    ...
    class AnswerSchema(BaseModel):
        ...
        relevant_sources: List[SourceRef] = Field(description="保持为空列表。")
...
# =============================================================================
# JSON repair (formatting-only helper)
# =============================================================================
...
```
{
  ...
  "relevant_sources": [{"file_name": "万科企业2022年年度报告.md", "line_start": 10, "line_end": 15}],
